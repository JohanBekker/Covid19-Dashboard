...
ic_opnames_leeftijd = ic_opnames_per_leeftijd.groupby('Age_group').sum().reset_index()

filepath3 = './files/df_gemeentes_per_dag.csv'
gevallen_per_gemeente_per_dag = pd.read_csv(filepath3)
gevallen_per_gemeente = gevallen_per_gemeente_per_dag.groupby('Province').sum().reset_index()
per_gemeente_grouped = gevallen_per_gemeente_per_dag.groupby(['Date_of_publication', 'Province']).sum().reset_index()
...
per_gemeente2['Province'].replace(['Fryslân'], 'Friesland (Fryslân)', inplace=True)
...
def update_graph(hdata, tabs):
    ...
    fig1.update_traces(marker_color='red', hoverinfo='skip', hovertemplate=None)
    fig1.add_trace(go.Scatter(x=df_data["Date_of_statistics"], 
                               y=df_data[data_dict[tabs]].rolling(window=7).mean().round(decimals=1), 
...
def update_graph2(hdata2, tabs):
    ...
    fig2.update_layout(showlegend=False, yaxis_title=None, xaxis_title=None, hovermode='x')
...
def update_graph3(hdata3, tabs):
    if tabs == 'ziekenhuis' or tabs == 'ic':
        fig3 = px.pie(ic_opnames_leeftijd, values=data_dict[tabs], names='Age_group', 
                      title='Totaal {} per Leeftijdsgroep'.format(card_dict[tabs]), 
...
def update_graph4(hdata4, tabs):
    if tabs == 'ziekenhuis' or tabs == 'ic':
        ...
        fig4.update_layout(yaxis_title=None, xaxis_title=None)#, hovermode='x')
    else:
...
def update_cards(n):
    ...
    global gevallen_per_gemeente_per_dag, gevallen_per_gemeente, per_gemeente_grouped, per_gemeente2
    global last_date_dict
